...
final class PaymentCredentialCipher
{
    ...
    public function encrypt(string $plainText, string $provider, string $credentialRef, string $purpose): string
    {
        ...
        $tag = '';
        $cipherText = openssl_encrypt($plainText, self::CIPHER, $this->key(), OPENSSL_RAW_DATA, $iv, $tag, $aad, self::GCM_TAG_LENGTH);
        ...
        return base64_encode(json_encode(['v'=>self::ENVELOPE_VERSION,'iv'=>base64_encode($iv),'tag'=>base64_encode($tag),'data'=>base64_encode($cipherText)], JSON_THROW_ON_ERROR));
    }
    ...
    public function decrypt(string $payload, string $provider, string $credentialRef, string $purpose): string
    {
        ...
        if (!is_array($decoded) || (int) ($decoded['v'] ?? 0) !== self::ENVELOPE_VERSION) {
            throw new RuntimeException('Versión de sobre cifrado no soportada.');
        }
        ...
        if ($iv === false || $tag === false || $data === false || strlen($tag) !== self::GCM_TAG_LENGTH) {
            throw new RuntimeException('Credencial cifrada dañada o tag GCM incompleto.');
        }
        $plainText = openssl_decrypt($data, self::CIPHER, $this->key(), OPENSSL_RAW_DATA, $iv, $tag, $this->aad($provider, $credentialRef, $purpose));
        if ($plainText === false) {
    ...
    {
        $provider = trim($provider); $credentialRef = trim($credentialRef); $purpose = trim($purpose);
        if ($provider === '' || $credentialRef === '' || $purpose === '') {
            throw new InvalidArgumentException('Proveedor, referencia y propósito son obligatorios.');
        }
        return json_encode(['schema'=>'hache-payment-credential-aad-v1','provider'=>$provider,'credential_ref'=>$credentialRef,'purpose'=>$purpose], JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES);
    }
